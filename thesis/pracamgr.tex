...
0.0                 & 0.7                 & 5355        & 0.9341            & 0.5227           \\ \hline
...
\hline
$threshold_{class}$ & $threshold_{drift}$ & Liczba cech & $X_{train}$ $BAC$ & $X_{test}$ $BAC$ \\ \hline
0.0                 & 0.7                 & 5355        & 0.9501            & 0.53             \\ \hline
...
\hline
$threshold_{class}$ & $threshold_{drift}$ & Liczba cech & $X_{train}$ $BAC$ & $X_{test}$ $BAC$ \\ \hline
0.0                 & 0.7                 & 5355        & 0.9952            & 0.7118           \\ \hline
...
\hline
$threshold_{class}$ & $threshold_{drift}$ & Liczba cech   & $X_{train}$ $BAC$ & $X_{test}$ $BAC$ \\ \hline
0.0                 & 8000                & 5355          & 0.9497            & 0.5348           \\ \hline
...
\hline
$threshold_{class}$ & $threshold_{drift}$ & Liczba cech   & $X_{train}$ $BAC$ & $X_{test}$ $BAC$ \\ \hline
0.0                 & 8000                & 5355          & 0.9586            & 0.5346           \\ \hline
...
\hline
$threshold_{class}$ & $threshold_{drift}$ & Liczba cech   & $X_{train}$ $BAC$ & $X_{test}$ $BAC$ \\ \hline
0.0                 & 8000                & 5355          & 0.9948            & 0.7118           \\ \hline
...
\hline
$threshold_{class}$ & $threshold_{drift}$ & Liczba cech   & $X_{train}$ $BAC$ & $X_{test}$ $BAC$ \\ \hline
0.0                 & 0.05                & 5355          & 0.9497            & 0.5348           \\ \hline
...
\hline
$threshold_{class}$ & $threshold_{drift}$ & Liczba cech  & $X_{train}$ $BAC$ & $X_{test}$ $BAC$ \\ \hline
0.0                 & 0.05                & 5355         & 0.9586            & 0.5346           \\ \hline
...
\hline
$threshold_{class}$ & $threshold_{drift}$ & Liczba cech  & $X_{train}$ $BAC$ & $X_{test}$ $BAC$ \\ \hline
0.0                 & 0.05                & 5355         & 0.9948            & 0.7118           \\ \hline
...
Problem adaptacji dziedziny w kontek¶cie analizowanego zbioru danych dotyczy ró¿nic w wykonywaniu tych samych czynno¶ci przez ró¿nych stra¿aków. Szukaj±c cech odpornych na ewolucjê pojêæ, naturalne by³oby wiêc wybieranie tych atrybutów, które najgorzej rozró¿niaj± stra¿aków miêdzy sob±. Problemem okaza³ siê jednak brak identyfikatora stra¿aka. W zamian dostêpna by³a inna informacja: liczba osób, od których zbierane by³y dane w ka¿dym ze zbiorów (gwoli przypomnienia: zbiór treningowy i testowy pochodzi³y od ró¿nych czteroosobowych grup stra¿aków). Wykorzystuj±c tê wiedzê, autor proponuje opart± o klasteryzacjê modyfikacjê metody opisanej na wstêpie rozdzia³u.
...
\begin{itemize}
\item metody hierarchiczne 
\item grupa metod k-centroidów
\item metody rozmytej analizy skupieñ
\end{itemize}

Autor u¿y³ implementacji algorytmu k-centroidów z biblioteki scikit-learn\footnote{http://scikit-learn.org}.

\subsubsection{Algorytm k-centroidów}
TODO

\subsection{Opis algorytmu detekcji dziedziny}

Maj±c dany identyfikator stra¿aka, sprowadzenie problemu wykrycia \textit{concept drift} do problemu klasyfikacji jest trywialne: wystarczy ów identyfikator potraktowaæ jako klasê wynikow±.
...
W poni¿szym rozdziale przedstawione zostan± dwie inne metody adaptacji dziedziny: uczenie iteracyjne oraz metoda zmiany przestrzeni cech zaproponowana przez Daume w (TODO biblio).

\section{Uczenie iteracyjne}

\subsection{Opis algorytmu}

Idea uczenia iteracyjnego jest stosunkowo intuicyjna. Polega na iteracyjnym powiêkszaniu zbioru treningowego na czê¶ci poprzednio sklasyfikowanych przyk³adów ze zbioru testowego. Bardziej szczegó³owo, uczenie iteracyjne dzia³a wed³ug nastêpuj±cego schematu:
...
\begin{lstlisting}[mathescape=true][frame=single][language=Python]
def iterative(self, classifier, X_train, y_train,
